...
def create_sentiment(path_data_in_pos, path_data_in_neg):
    sentiment = []
    with open(path_data_in_pos, 'rb') as data_in_pos:
        for txt_pos in data_in_pos:
            sentiment.append('+1 ' + txt_pos.decode('utf-8', 'ignore').strip())

    with open(path_data_in_neg, 'rb') as data_in_neg:
        for txt_neg in data_in_neg:
            sentiment.append('-1 ' + txt_neg.decode('utf-8', 'ignore').strip())
