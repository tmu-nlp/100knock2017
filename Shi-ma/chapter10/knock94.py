...
if __name__ == '__main__':
    ...
    with open('../data/wordsim353/combined.tab', 'r') as data_in:
        with open('result/knock94_85_result.txt', 'w') as data_out_85, open('result/knock94_90_result.txt', 'w') as data_out_90:
            ...
            for num_line, line in enumerate(data_in):
                if num_line == 0:
                    ...
                    if not all(map(lambda x: x in ids_t.keys(), cols)) or not all(map(lambda x: x in X100_90, cols)):
                        continue
                    simi_85 = np.dot(X_100_85[ids_t[cols[0]]], X_100_85[ids_t[cols[1]]])
