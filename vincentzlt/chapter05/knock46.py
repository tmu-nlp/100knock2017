        ...
        for p in self.tree_d:
            ...
            if father_p != -1:
                ...
                if father_p_pos == "動詞":
                    ...
                    particle=self.tree_d[p]["words"][len_p - 1]['表層形']
                    phrase=self.print_phrase(p)
                    if not predicate in output:
                        output[predicate]=[[],[]]
                        output[predicate][0]=[particle]
                        output[predicate][1]=[phrase]
                    else:
                        output[predicate][0].append(particle)
                        output[predicate][1].append(phrase)
        for i in output:
            print(i,end="\t")
            for j in output[i]:
                print(" ".join(j),end="\t")
