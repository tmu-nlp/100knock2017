...
class cabocha_tree(object):
    ...
    def print_knock42(self):
        for p in self.tree_d:
            father=int(self.tree_d[p]["father"][:-1])
            for w in self.tree_d[p]["words"]:
                if self.tree_d[p]["words"][w]["品詞"]!="記号":
                    print(self.tree_d[p]["words"][w]['原形'],end="")
            print("\t",end="")
            if father!=-1:
                for w in self.tree_d[father]["words"]:
                    if self.tree_d[father]["words"][w]["品詞"]!="記号":
                        print(self.tree_d[father]["words"][w]['原形'],end="")
            print()
